...
after defining the schema, table, and variables for our database we can now create the database and its tables using the function create_db_and_tables and the code inside that function is another chain of functions from sqlmodel SQLModel.metadata.create_all(), the create_all() function uses the/our engine that we assigned earlier.
...
finally we need to get the session and its dependency, we can do that by calling the contextmanager using @contextmanager and creating a funtion called get_session() using the engine as our Session and assigning it as session. and for getting the session dependency we create another function called get_session_depency and inside it we use engine as our Session and assign it as session, and now we yield session


missed information:

Middleware's importance:
not only does it log all requests but Middleware also intercepts ALL HTTP requests and responses. It also works before AND after enpoints run. It is also applied globally, and not per router.

why CORSMiddleware is required for frontend-backend connection:
Brosers block requests from different origins as a security measure. CORS tells the browsere which origins are trusted. Without CORSMiddleware the React app cannot call FastAPI, it serves as both as a bridge to connect a place to another and also as a barrier/immune system to block viruses and attacks.

the use of include_router:
include_router keeps code modular, prefix="/api" keeps endpoint paths clean and consistent, with it I can also include the version of my api in the future (e.g., /api/v1)